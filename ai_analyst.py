...
def ask_chatgpt(name, company, category, ingredient, efficacy_text=""):
    ...
    # 신약/자료제출의약품/유전자치료제/희귀인 경우 추출된 효능효과 데이터를 프롬프트에 추가
    if efficacy_text:
        prompt += f"\n- [중요 분석 조건]: 이 의약품은 신약, 유전자치료제, 희귀 또는 자료제출의약품입니다. 아래 실제 효능효과 텍스트를 분석하여 분류의 최우선 근거로 삼으세요.\n[효능효과 데이터]: {efficacy_text}\n"
...
def main():
    ...
    for item in pending:
        ...
        if not final_cat:
            ...
            efficacy_text = ""
            if ("신약" in approval_type or "자료제출의약품" in approval_type or "유전자치료제" in approval_type or "희귀" in approval_type) and detail_link:
