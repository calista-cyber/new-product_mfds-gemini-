...
def run_scraper():
    print(f"=== 🚀 다이렉트 수집 시작 ({str_start} ~ {str_end}) ===")
    ...
    for row in rows:
        ...
        # ✨ [개선] 진짜 날짜(숫자)가 포함된 경우만 '취하'로 판단하여 건너뜀
        cancel_text = cols[4].text.strip()
        if re.search(r'\d', cancel_text): 
        ...
        try:
            ...
            api = get_api_data(item_seq) or {
                "name": cols[1].text.strip(), "ingr": "API 확인 필요",
                "com": cols[2].text.strip(), "date": cols[3].text.strip(), "cat": cols[5].text.strip()
            }
            ...
            # USER_ENTERED 옵션을 주어야 하이퍼링크 수식이 작동함
